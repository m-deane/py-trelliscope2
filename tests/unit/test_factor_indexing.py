...
class TestFactorIndexingConversion:
    ...
    def test_non_factor_values_unchanged(self):
        ...
        display.add_meta_variable(
            FactorMeta(varname="country", label="Country", levels=['Algeria', 'Denmark'])
        )
        display.add_meta_variable(NumberMeta(varname="value", label="Value"))
    ...
    def test_multiple_factor_columns(self):
        ...
        display.add_meta_variable(
            FactorMeta(varname="status", label="Status",
    ...
    def test_factor_with_none_values(self):
        ...
        assert second_value != second_value or second_value is None, "NaN/None values should be preserved"
    ...
    def test_string_not_in_levels_preserved(self):
        ...
        assert cog_data[1]['country'] == 'Unknown', "String not in levels should be preserved"
...
class TestMetadataJsonFactorIndexing:
    ...
    def test_metadata_json_preserves_other_values(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            display.add_meta_variable(
                FactorMeta(varname="country", label="Country",
                          levels=['Algeria', 'Denmark'])
            )
            display.add_meta_variable(NumberMeta(varname="avg_value", label="Average"))
            display.add_meta_variable(NumberMeta(varname="count", label="Count", digits=0))
...
class TestMetadataJsFactorIndexing:
    ...
    def test_metadata_js_has_1based_factors(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert content.startswith('window.metaData = '), "Should have window.metaData wrapper"
...
class TestEndToEndFactorIndexing:
    ...
    def test_full_display_write_with_factors(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            displayinfo_path = display_path / "displays" / "refinery_test" / "displayInfo.json"
            with open(displayinfo_path) as f:
                info = json.load(f)

            assert info['cogData'][0]['country'] == 1, "Algeria should be 1 in displayInfo"

            # Verify metaData.json has 1-based factors
            metadata_json_path = display_path / "displays" / "refinery_test" / "metaData.json"
            with open(metadata_json_path) as f:
                metadata_json = json.load(f)

            assert metadata_json[0]['country'] == 1, "Algeria should be 1 in metaData.json"

            # Verify metaData.js has 1-based factors
            metadata_js_path = display_path / "displays" / "refinery_test" / "metaData.js"
    ...
    def test_viewer_compatibility(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Load displayInfo
            displayinfo_path = display_path / "displays" / "viewer_compat_test" / "displayInfo.json"
            with open(displayinfo_path) as f:
                info = json.load(f)
            ...
            for entry in info['cogData']:
                assert isinstance(entry['status'], int), "Viewer expects integer indices"
...
class TestFactorIndexingEdgeCases:
    ...
    def test_single_row(self):
        ...
        df = pd.DataFrame({
            'country': pd.Categorical(['Algeria'])
        })
    ...
    def test_large_number_of_levels(self):
        ...
        df = pd.DataFrame({
            'category': pd.Categorical(categories, categories=levels)
        })
    ...
    def test_unicode_factor_levels(self):
        ...
        df = pd.DataFrame({
            'country': ['中国', 'Россия', 'العربية']
        })

        display = Display(df, name="test_unicode")
        display.add_meta_variable(
            FactorMeta(varname="country", label="Country",
                      levels=['中国', 'Россия', 'العربية'])
        )
